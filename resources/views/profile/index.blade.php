                    ...
                    <div class="card-body text-center">
                        @if ($user->profile_photo)
                            <img src="{{ asset('storage/' . $user->profile_photo) }}"
                                class="rounded-circle mb-4 border border-primary" alt="Profile Photo" width="120"
                                height="120">
                        @else
                            <img src="https://via.placeholder.com/150" class="rounded-circle border border-secondary"
                            ...
                            @foreach ($mediaUploads as $media)
                                <div class="col-md-{{ 12 / $feedPerRow }} mb-3"> <!-- Dinamis per row -->
                                    <div class="card shadow-sm" style="height: 100%;"> <!-- Card height tetap -->
                                        @if (in_array($media->file_type, ['jpg', 'jpeg', 'png', 'mov', 'mp4']))
                                            <img src="{{ asset('storage/' . $media->file_path) }}" class="card-img-top"
                                                alt="media">
                                        @elseif(in_array($media->file_type, ['']))
                                        ...
                                        <div class="card-body d-flex flex-column">
                                            ...
                                            <form action="{{ route('profile.media.delete', $media->id) }}" method="POST"
                                                style="display:inline;" id="delete-form-{{ $media->id }}">
                                                @csrf
                                                @method('DELETE')
                                                <div class="input-group mb-3">
                                                    <select class="form-select w-100" name="action"
                                                        onchange="handleActionChange(this, '{{ $media->id }}');">
                                            ...
                                            <form action="{{ route('profile.media.update', $media->id) }}" method="POST"
                                                id="edit-caption-form-{{ $media->id }}" style="display:none;"
                                                class="mt-2">
                                                @csrf
                                                @method('PUT')
                                                <div class="col-md-13">
                                                    <input type="text" name="caption" value="{{ $media->caption }}"
                                                        class="form-control" required>
                                                </div>
                                                <div class="col-md-13 mt-2">
                                                    <button type="button" class="btn btn-primary"
                                                        onclick="confirmEdit('{{ $media->id }}')">Submit</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
